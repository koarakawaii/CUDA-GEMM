...
#define T_ELEM_IN       double
#define T_ELEM_OUT      double
#define CUBLAS_ELEM_IN  CUDA_R_64F
#define CUBLAS_ELEM_OUT CUDA_R_64F
#define TEST_IDX     2097152
#define WARPSIZE     32
#define THREADTILE   4
#define THREADTILE_X 4
#define THREADTILE_Y 2

//// for RTX5090
#define BM_Warptiling 32
#define BK_Warptiling 8
#define BN_Warptiling 32
#define WM_Warptiling 16
#define WN_Warptiling 32
#define TM_Warptiling 2
#define TN_Warptiling 2
#define WMITER        2
#define WNITER        2
const int WARP_TILING_MAX_NUM_THREADS = 64;
////
//// for RTX3080Ti
//#define BM_Warptiling 32
//#define BK_Warptiling 8
                            ...
                            T_ELEM_OUT alpha, T_ELEM_OUT beta)
{
...
#ifndef PERFORMANCE
    if ( lda%2 != 0 ) // lda must be multiple of 2 for data type double
    {
        assert(0);
    }
    if ( ldb%2 != 0 ) // ldb must be multiple of 2 for data type double
    {
        assert(0);
    }
    if ( ldc%2 != 0 ) // ldc must be multiple of 2 for data type double
    {
        assert(0);
    }
#endif
    ...
    int threadtile_idx_y = (THREADTILE_WMMA_Y*threadIdx.y) + BM*blockIdx.y;     // BM = THREADTILE_WMMA_Y*blockDim.y
    //int threadtile_idx_y = threadIdx.y + BM*blockIdx.y;     // BM = THREADTILE_WMMA_Y*blockDim.y
    while ( threadtile_idx_y < (BM * (int)((M + BM - 1) / BM)) )
    {
        ...
        int threadtile_idx_x = threadIdx.x + BN*blockIdx.x; // BN = THREADTILE_WMMA_X*blockDim.x
        while ( threadtile_idx_x < (BN * (int)((N + BN - 1) / BN)) )
        {
            ...
            for (int i=0; i<K; i+=(warpK_stride_shared*WMMA_K))
            {
                ...
                while (thread_idx_x < (warpK_stride_shared*WMMA_K))
                {
                    ...
                    for (int inner_idx=0; inner_idx<THREADTILE_WMMA_Y; ++inner_idx)
                    {
                        ...
                        //int aRow_shared                                                            = threadIdx.y+blockDim.y*inner_idx;
                        int aRow                                                                   = aRow0 + inner_idx;
                        //int aRow                                                                   = aRow0 + blockDim.y*inner_idx;
                        if ( (aRow < M) && (aCol < K) )
                            a_shared_buffer[thread_idx_x+aRow_shared*(warpK_stride_shared*WMMA_K)] = a[aCol + aRow*lda];
                        else
                            a_shared_buffer[thread_idx_x+aRow_shared*(warpK_stride_shared*WMMA_K)] = 0.0;
                    }
                    thread_idx_x                                                                      += blockDim.x;
                }
                int thread_idx_y  = threadIdx.y;
                while (thread_idx_y < (warpK_stride_shared*WMMA_K))
                {
                    ...
                    for (int inner_idx=0; inner_idx<THREADTILE_WMMA_X; ++inner_idx)
                    {
                        ...
                        // in order to realize memory coalescing for threads in the same wrap, we use threadIdx.x+blockDim.x*inner_idx instead of THREADTILE_WMMA_X*threadIdx.x+inner_idx for mapping along row direction for b
                        if ( (bRow < K) && (bCol < N) )
                            b_shared_buffer[bCol_shared+thread_idx_y*BN] = b[bCol + bRow*ldb];
                        else
                            b_shared_buffer[bCol_shared+thread_idx_y*BN] = 0.0;
                    }
                    thread_idx_y                                        += blockDim.y;
                }
                __syncthreads();
            ...
            int cRow = aRow0 - (THREADTILE_WMMA_Y*threadIdx.y) + (warp_idx/wpB_x)*WMMA_M;
            //int cRow = aRow0 - threadIdx.y                     + (warp_idx/wpB_x)*WMMA_M;
            int cCol = bCol0 - threadIdx.x                     + (warp_idx%wpB_x)*WMMA_N;
            if (cRow < M && cCol < N)
            {
...
//                       a_shared_buffer[thread_idx] = a[ aCol_cache + aRow_cache * lda ];
//                   else
//                       a_shared_buffer[thread_idx] = 0.0;
...
//                       b_shared_buffer[thread_idx] = b[ bCol_cache + bRow_cache * ldb ];
//                   else
//                       b_shared_buffer[thread_idx] = 0.0;
//                   thread_idx                     += blockDim.x * blockDim.y;
//                }
//                __syncthreads();
//
...
int main(int argc, char* argv[])
{
    ...
    std::vector<std::normal_distribution<double>> norm_dist(N_thread);

    for (int idx_thread=0; idx_thread<N_thread; ++idx_thread)
    {
        generator[idx_thread] = std::mt19937(seed + (long)(idx_thread));
        norm_dist[idx_thread] = std::normal_distribution<double>(0.0, 1.0);
    }
    ...
    */
    ...
    int BM_WMMA = WMMA_M * wpB_y;
    int BN_WMMA = WMMA_N * wpB_x;
    ...
    CUDA_CHECK_ERROR(cudaEventRecord(t_start,0));
#endif
    shmem_size = (WMMA_M * wpB_y + WMMA_N * wpB_x) * (warpK_stride_shared * WMMA_K) * sizeof(T_ELEM_IN);
    if ( shmem_size > shmem_size_max )
    {
        printf("Shared memory size (%lu) > Maximum shared Memory size (%lu) !! Exit!!\n", shmem_size, shmem_size_max);
        return EXIT_FAILURE;
    ...
    */
    ...
    // K, N, lda, ldb, ldc and are all multiples of 2 for double precision in our case for GemmEx:
    if ( K%2 != 0 )
    {
        assert(0);
    }
    ...
    //CUBLAS_CHECK_ERROR(cublasSetMathMode(handle, cublasMath_t(CUBLAS_FP64_EMULATED_FIXEDPOINT_MATH | CUBLAS_MATH_DISALLOW_REDUCED_PRECISION_REDUCTION)));
    ...
    // Invoke the GEMM, ensuring k, n(since now our B is the A in the example), ldb, lda, and ldc are all multiples of 2 for double precision, i.e. 16 bytes
    ...
    CUBLAS_CHECK_ERROR(cublasGemmEx(handle, trans_b, trans_a,
                                    ...
                                    //CUBLAS_COMPUTE_64F_EMULATED_FIXEDPOINT, CUBLAS_GEMM_DEFAULT_TENSOR_OP));
    ...
    */
    ...
    trans_b = CUBLAS_OP_N;
    trans_a = CUBLAS_OP_N;
    CUBLAS_CHECK_ERROR(cublasDgemm(handle, trans_b, trans_a,
                                   N,               M,               K,   &alpha,
                                   matrix_b_device, ldb,
                                   matrix_a_device, lda,
                                   &beta,
    ...
    diff = sqrt(diff / ((double)M * (double)N));
    norm = sqrt(norm / ((double)M * (double)N));
    printf("cuBLAS Dgemm relative L2 error between matrix_c_host and matrix_c_host is %.4e .\n", diff/norm);
#ifdef TIMING_FLAG
    CUDA_CHECK_ERROR(cudaEventRecord(t_stop,0));
    CUDA_CHECK_ERROR(cudaEventSynchronize(t_stop));
    CUDA_CHECK_ERROR(cudaEventElapsedTime(&elapsed_time, t_start, t_stop));
