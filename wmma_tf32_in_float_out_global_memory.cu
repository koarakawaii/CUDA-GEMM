...
__global__ void float_to_tf32_kernel(float *a_out, float *b_out, float *a_in, float *b_in,
                                     int M, int N, int K)
{
    const int lda   = K;
    const int ldb   = K;

    int array_idx_x, array_idx_y;

    array_idx_y = threadIdx.y + blockDim.y * blockIdx.y;
    while ( array_idx_y < M )
    {
        array_idx_x = threadIdx.x + blockDim.x * blockIdx.x;
        while ( array_idx_x < K )
        {
            a_out[ array_idx_y * lda + array_idx_x ] = wmma::__float_to_tf32(a_in[ array_idx_y * lda + array_idx_x ]);
    ...
    while ( array_idx_y < N )
    {
        ...
        while ( array_idx_x < K )
        {
            b_out[ array_idx_y * ldb + array_idx_x ] = wmma::__float_to_tf32(b_in[ array_idx_y * ldb + array_idx_x ]);
...
__global__ void gemm_kernel_float4(float *a, T_ELEM_IN *b, T_ELEM_OUT *c,
                                   int M, int N, int K,
                                   T_ELEM_OUT alpha, T_ELEM_OUT beta)
{
    const int     lda      = K;
    ...
    while ( array_idx_y < M )
    {
        ...
        while ( array_idx_x < N )
        {
            ...
            {
                #pragma unroll
                for (int j=0; j<(warpK_stride*WMMA_K)/STRIDE_4; ++j)
                {
                    a4_temp = reinterpret_cast<float4 *>(a + aRow*lda + (STRIDE_4*j+i));
                    acc    +=   (T_ELEM_OUT)a4_temp[0].x * (T_ELEM_OUT)b[bCol + (STRIDE_4*j+i+0)*ldb]
                              + (T_ELEM_OUT)a4_temp[0].y * (T_ELEM_OUT)b[bCol + (STRIDE_4*j+i+1)*ldb]
                              + (T_ELEM_OUT)a4_temp[0].z * (T_ELEM_OUT)b[bCol + (STRIDE_4*j+i+2)*ldb]
                              + (T_ELEM_OUT)a4_temp[0].w * (T_ELEM_OUT)b[bCol + (STRIDE_4*j+i+3)*ldb];
                }
            }
...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    ////* GPU hand-carved GEMM-float4 computation *////
    if ( (warpK_stride*WMMA_K) % STRIDE_4 != 0)
    {
        printf("Error!! warpK_stride*WMMA_K(%d) must be divisible by %d!! Exit!!\n", warpK_stride*WMMA_K, STRIDE_4);
        return EXIT_FAILURE;
    }
    printf("2D blocks per grid is: (%d,%d) for hand-carved GEMM-float4.\n", bpG_x_GEMM, bpG_y_GEMM);
#ifdef TIMING_FLAG
    CUDA_CHECK_ERROR(cudaEventRecord(t_start,0));
#endif
    CUDA_CHECK_ERROR(cudaMemcpy(matrix_c_device, matrix_c_host_backup.data(), (size_t)M*(size_t)N*sizeof(T_ELEM_OUT), cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK_ERROR(cudaEventRecord(t_start,0));
#endif
    gemm_kernel_float4<<<bpG_gemm, tpB>>>(matrix_a_device, matrix_b_device, matrix_c_device,
                                          M, N, K,
                                          (T_ELEM_OUT)alpha, (T_ELEM_OUT)beta);
    CUDA_CHECK_ERROR(cudaDeviceSynchronize());
    CUDA_CHECK_ERROR(cudaPeekAtLastError());
    ...
    CUDA_CHECK_ERROR(cudaEventSynchronize(t_stop));
    CUDA_CHECK_ERROR(cudaEventElapsedTime(&elapsed_time, t_start, t_stop));
    printf("CUDA hand-carved matrix-matrix multiplication with float4 time = %.4f s\n", elapsed_time/ms_to_sec);
#endif
    ...
    diff = sqrt(diff / ((double)M * (double)N));
    norm = sqrt(norm / ((double)M * (double)N));
    printf("Hand-carved GEMM-float4 relative L2 error between matrix_c_buffer and matrix_c_host is %.4e .\n", diff/norm);
#ifdef TIMING_FLAG
    CUDA_CHECK_ERROR(cudaEventRecord(t_stop,0));
    CUDA_CHECK_ERROR(cudaEventSynchronize(t_stop));
    CUDA_CHECK_ERROR(cudaEventElapsedTime(&elapsed_time, t_start, t_stop));
    ...
    CUDA_CHECK_ERROR(cudaEventRecord(t_start,0));
#endif
    float_to_tf32_kernel<<<bpG_WMMA, tpB>>>(matrix_a_device, matrix_b_device, matrix_a_device, matrix_b_device,
                                            M, N, K);
