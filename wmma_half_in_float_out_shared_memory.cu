...
//
//                   //int aRow_cache = warpM0 * WMMA_M + thread_idx%(WMMA_M*wpB_y);
//                   //int aCol_cache = aCol + thread_idx/(WMMA_M*wpB_y);
//                   //if ( (aRow_cache < M) && (aCol_cache < K) )
//                   //    a_shared_buffer[thread_idx] = a[ aCol_cache + aRow_cache * lda ];
//                   //else
//                   //    a_shared_buffer[thread_idx] = (T_ELEM_IN)0.0;
...
//                            // Load the inputs
//                            wmma::load_matrix_sync(a_frag, a_shared_buffer + j + (warp_idx/wpB_x)*WMMA_M*(warpK_stride_shared*WMMA_K), warpK_stride_shared*WMMA_K);
//                            //wmma::load_matrix_sync(a_frag, a_shared_buffer + (warp_idx/wpB_x)*WMMA_M + j*(WMMA_M*wpB_y), WMMA_M*wpB_y);
//                            wmma::load_matrix_sync(b_frag, b_shared_buffer + (warp_idx%wpB_x)*WMMA_N + j*(WMMA_N*wpB_x), WMMA_N*wpB_x);
...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    CUDA_CHECK_ERROR(cudaEventSynchronize(t_stop));
    CUDA_CHECK_ERROR(cudaEventElapsedTime(&elapsed_time, t_start, t_stop));
    printf("CUDA hand-carved matrix-matrix with 1D tiling multiplication time = %.4f s\n", elapsed_time/ms_to_sec);
#endif
    ...
    diff = sqrt(diff / ((double)M * (double)N));
    norm = sqrt(norm / ((double)M * (double)N));
    printf("Hand-carved GEMM_1DTiling relative L2 error between matrix_c_float_host and matrix_c_host is %.4e .\n", diff/norm);
#ifdef TIMING_FLAG
    CUDA_CHECK_ERROR(cudaEventRecord(t_stop,0));
    CUDA_CHECK_ERROR(cudaEventSynchronize(t_stop));
    CUDA_CHECK_ERROR(cudaEventElapsedTime(&elapsed_time, t_start, t_stop));
    ...
    diff = sqrt(diff / ((double)M * (double)N));
    norm = sqrt(norm / ((double)M * (double)N));
    printf("Hand-carved GEMM_2DTiling relative L2 error between matrix_c_float_host and matrix_c_host is %.4e .\n", diff/norm);
#ifdef TIMING_FLAG
    CUDA_CHECK_ERROR(cudaEventRecord(t_stop,0));
    CUDA_CHECK_ERROR(cudaEventSynchronize(t_stop));
    CUDA_CHECK_ERROR(cudaEventElapsedTime(&elapsed_time, t_start, t_stop));
    ...
    diff = sqrt(diff / ((double)M * (double)N));
    norm = sqrt(norm / ((double)M * (double)N));
    printf("Hand-carved GEMM_WarpTiling relative L2 error between matrix_c_float_host and matrix_c_host is %.4e .\n", diff/norm);
#ifdef TIMING_FLAG
    CUDA_CHECK_ERROR(cudaEventRecord(t_stop,0));
    CUDA_CHECK_ERROR(cudaEventSynchronize(t_stop));
    CUDA_CHECK_ERROR(cudaEventElapsedTime(&elapsed_time, t_start, t_stop));
    ...
    CUDA_CHECK_ERROR(cudaEventRecord(t_start,0));
#endif
    shmem_size = (BM_WMMA*(warpK_stride_shared*WMMA_K) + (warpK_stride_shared*WMMA_K)*BN_WMMA) * sizeof(T_ELEM_IN);
    if ( shmem_size > shmem_size_max )
    {
        printf("Shared memory size (%lu) > Maximum shared Memory size (%lu) !! Exit!!\n", shmem_size, shmem_size_max);
        return EXIT_FAILURE;
